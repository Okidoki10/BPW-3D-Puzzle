...
public class AN_DoorScript : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        HeroInteractive = GameObject.FindGameObjectWithTag("Player");
    }
    ...
    public void Action() // void to open/close door
    {
        if (!Locked)
        {
            ...
            {
                isOpened = true;
                rbDoor.AddRelativeTorque(new Vector3(0, 0, -50f)); 
            }
    ...
    {
        distance = Vector3.Distance(transform.position, HeroInteractive.transform.position);
